...
# 仓库信息
REPO_URL="https://github.com/vonssy/StreamAi-BOT.git"
REPO_DIR="StreamAi-BOT"
...
# 清理 userids.txt 文件
echo "清理 userids.txt 文件..."
echo '' > userids.txt

# 交互式输入 UUID
echo "请输入 UUID，每行一个，输入完成后按回车结束："
while true; do
  read -p "UUID（留空结束）：" UUID
  if [ -z "$UUID" ]; then
    break
  fi

  # 验证 UUID 格式（可以根据需要调整正则表达式）
  if [[ "$UUID" =~ ^[0-9a-fA-F-]{36}$ ]]; then
    echo "添加 UUID：$UUID"
    echo "$UUID" >> userids.txt
  else
    echo "无效的 UUID 格式，请输入有效的 UUID。"
  fi
done
